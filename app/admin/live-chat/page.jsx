...
function TypingIndicator({ pm, label }) {
  return (
    <div className="flex items-center gap-2 px-1 text-[11px] font-black tracking-widest uppercase" style={{ color: pm.fg70 }}>
      <span>{label}</span>
...
function ChatModal({ open, onClose, pm, activeThread, messages, loadingMessages, text, onChangeText, sending, send, listRef, peerTyping }) {
      ...
      <div className="flex h-[min(86vh,760px)] w-full max-w-2xl flex-col overflow-hidden border shadow-2xl" style={{ borderColor: pm.b28, background: "var(--pm-bg)", color: pm.fg }}>
        ...
        <div ref={listRef} className="h-[460px] flex-1 space-y-2 overflow-y-auto px-4 py-3">
          {loadingMessages ? <div className="text-sm">Loading...</div> : messages.length === 0 ? <div className="flex h-full items-center justify-center text-sm" style={{ color: pm.fg70 }}>No messages yet.</div> : messages.map((m) => {
            ...
            return <div key={m._id} className={`flex ${mine ? "justify-end" : "justify-start"}`}><div className="max-w-[85%] border px-3 py-2 text-sm" style={{ borderColor: pm.b20, background: mine ? pm.greenBg : pm.bg08 }}><div className="text-[10px] font-black tracking-widest uppercase" style={{ color: pm.fg70 }}>{mine ? "Admin" : "User"}</div>{m.message ? <div className="mt-1 whitespace-pre-wrap">{m.message}</div> : null}{m.imageUrl ? <img src={m.imageUrl} alt="Chat upload" className="mt-2 max-h-40 rounded border object-cover pointer-events-none select-none" style={{ borderColor: pm.b20 }} /> : null}<div className="mt-1 text-[10px]" style={{ color: pm.fg70 }}>{m.createdAt ? new Date(m.createdAt).toLocaleString() : ""}</div></div></div>;
          })}
        </div>
    ...
    <div className={`${funnelDisplay.className} min-h-screen px-4 py-6 pt-16 md:pt-6`} style={{ background: "var(--pm-bg-grad)", color: pm.fg, fontFamily: "var(--pm-font)" }}>
      <div className="mx-auto max-w-md space-y-3">
        ...
        <div className="space-y-2">
          {loadingThreads ? <div className="border px-3 py-4 text-sm" style={{ borderColor: pm.b28, background: pm.bg06 }}>Loading...</div> : threads.length === 0 ? <div className="border px-3 py-4 text-sm" style={{ borderColor: pm.b28, background: pm.bg06, color: pm.fg70 }}>No chat users yet.</div> : threads.map((thread) => <button key={thread._id} type="button" onClick={() => openThread(thread)} className="w-full border px-3 py-3 text-left" style={{ borderColor: pm.b28, background: pm.bg06 }}><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="text-sm font-black truncate">{thread?.user?.fullName || "User"}</div><div className="mt-1 text-[11px] truncate" style={{ color: pm.fg70 }}>{thread?.user?.mobile || ""}</div></div><div className="text-right">{Number(thread.unreadByAdmin || 0) > 0 ? <div className="inline-flex border px-2 py-1 text-[10px] font-black tracking-widest uppercase" style={{ borderColor: pm.b28, background: pm.bg10 }}>NEW</div> : null}</div></div></button>)}
        </div>
      </div>
      <ChatModal open={open} onClose={() => setOpen(false)} pm={pm} activeThread={activeThread} messages={messages} loadingMessages={loadingMessages} text={text} onChangeText={onChangeText} sending={sending} send={send} listRef={listRef} peerTyping={peerTyping} />
    </div>
  );
}
