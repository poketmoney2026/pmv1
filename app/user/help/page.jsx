...
export default function UserHelpPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await fetch("/api/help", { credentials: "include", cache: "no-store" });
        const json = await res.json().catch(() => ({}));
        if (!live) return;
        if (!res.ok || !json?.ok) return toast.error(json?.message || "Failed to load help");
        setData({ title: json?.data?.title || "ব্যবহার নির্দেশিকা", points: Array.isArray(json?.data?.points) ? json.data.points : [] });
      } catch {
        ...
        <div className="border p-3" style={{ borderColor: pm.b28, background: pm.bg06 }}>
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-lg font-black tracking-widest uppercase">{data.title || "ব্যবহার নির্দেশিকা"}</div>
              <div className="mt-2 text-[11px]" style={{ color: pm.fg70 }}>এই পেজ দেখে আপনি পুরো সিস্টেম কীভাবে ব্যবহার করতে হয় তা সহজে বুঝতে পারবেন।</div>
            </div>
