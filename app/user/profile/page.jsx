    ...
    <div className="min-h-screen mt-14 px-3 py-4 font-medium" style={{ background: "var(--pm-bg-grad)", color: pm.fg }}>
      <div className="mx-auto w-full max-w-sm space-y-2">
        <Block title="PROFILE OVERVIEW">
          <div className="flex items-start justify-between gap-3"><div><div className="text-[10px]" style={{ color: pm.fg70 }}>ACCOUNT</div><div className="mt-1 text-lg font-black tracking-widest uppercase">{u.role === "admin" ? "ADMIN" : u.role === "agent" ? "AGENT" : "USER"}</div><div className="mt-2 inline-flex items-center gap-2 text-[11px]" style={{ color: pm.fg90 }}><CalendarDays className="h-4 w-4" style={{ color: pm.fg80 }} />Joined: <span className="font-black">{formatJoinDate(u.joinDate)}</span></div><div className="mt-3 inline-flex items-center gap-2 border px-3 py-1 text-[11px] font-black tracking-widest uppercase" style={{ borderColor: pm.b28, background: pm.bg08 }}><BadgeCheck className="h-4 w-4" />PROFILE</div></div><span className="grid h-12 w-12 place-items-center border" style={{ borderColor: pm.b28, background: pm.bg08 }}><User className="h-6 w-6" /></span></div>
        </Block>
